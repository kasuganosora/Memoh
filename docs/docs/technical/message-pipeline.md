...
> **摘要**：本文介绍 Memoh 消息处理管线的设计与实现。通过引入**两阶段心跳**消除 LLM 自主发消息的安全隐患，建立 **Budget Model** 统一后台任务模型降低成本 70-90%，搭建**多模态回退管线**让 DeepSeek 等纯文本模型也能"看懂"图片，最终将硬编码的消息处理逻辑重构为**可插拔的 `MessageProcessor` 接口**——新增内容类型只需实现接口加一行注册，无需修改现有代码。全文覆盖架构决策、根因分析和 31 个文件的实现细节。
